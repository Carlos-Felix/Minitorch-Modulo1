...
class FunctionBase:
    ...
    def chain_rule(cls, ctx, inputs, d_output):
        ...
        l = []
        bv = cls.backward(ctx, d_output)

        if isinstance(bv, tuple) is False:
            bv = (bv)

        for i, v in enumerate(inputs):
            if (isinstance(v, int) is False) and (isinstance(v, float) is False):
                if v.history is not None:
                    l.append(VariableWithDeriv(v, bv[i]))
...
def backpropagate(final_variable_with_deriv):
    ...
    q = []
    q.append(final_variable_with_deriv)

    while len(q) != 0:

        varDer = q.pop(0)

        if varDer.variable.history.is_leaf():
            varDer.variable._add_deriv(varDer.deriv)

        else:
            v_d = varDer.variable.history.last_fn.chain_rule(varDer.variable.history.ctx, varDer.variable.history.inputs, varDer.deriv)
